...
interface TransformdErrorObject {
  name: string
  property: string
  message: string
...
function toErrorSchema(errors: TransformdErrorObject[]) {
...
function transformErrors(errors: Ajv.ErrorObject[] | null | undefined) {
  ...
  return (errors as any).map(
    ({ message, dataPath, keyword, params, schemaPath }) => {
      return {
        name: keyword,
        property: `${dataPath}`,
        message,
